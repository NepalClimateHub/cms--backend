...
class EmailService {
  ...
  constructor(private configService?: ConfigService) {
    this.config = {
      host: this.configService?.get('smtp.host') || process.env.SMTP_HOST || 'smtp.gmail.com',
      port: this.configService?.get('smtp.port') || parseInt(process.env.SMTP_PORT || '587'),
      secure: this.configService?.get('smtp.secure') || process.env.SMTP_PORT === '465',
      auth: {
        user: this.configService?.get('smtp.user') || process.env.SMTP_USER || '',
        pass: this.configService?.get('smtp.pass') || process.env.SMTP_PASS || '',
      },
  ...
  private async getTemplate(templateName: string): Promise<string> {
    ...
    try {
      ...
      throw new Error(`Template not found: ${templateName}.hbs at path: ${templatePath}`)
    }
  }
  ...
  private getSubject(emailType: EmailType, metadata: EmailMetadata): string {
    const subjects = {
      ...
      [EmailType.EMAIL_VERIFIED]: 'Email Verified Successfully - Nepal Climate Hub',
      [EmailType.PASSWORD_RESET]: 'Reset Your Password - Nepal Climate Hub',
      [EmailType.PASSWORD_RESET_SUCCESS]: 'Password Reset Successful - Nepal Climate Hub',
    }
  ...
  private prepareTemplateData(emailType: EmailType, metadata: EmailMetadata): Record<string, any> {
    const baseData = {
...
export const sendEmail = (
  ...
  return emailService.sendEmail(emailType, metadata)
}
